#para descargar glove: (en terminal)
...
glove_file = "/home/xnmaster/testproject/XNAPproject-grup-04/glove.6B.300d.txt"
embeddings_index = {}
...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, lang, embeddings_index, embed_size, dropout_p=0.1):
        ...
        self.embedding.weight.data.copy_(torch.from_numpy(
            self.create_embedding_matrix(input_size, embeddings_index, embed_size)))

        self.gru = nn.GRU(embed_size, hidden_size, batch_first=True)
        self.dropout = nn.Dropout(dropout_p)

    def create_embedding_matrix(self, input_size, embeddings_index, embed_size):
        matrix = np.zeros((input_size, embed_size))
        for word, idx in self.lang.word2index.items():
...
class DecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, lang, embeddings_index, embed_size):
        super(DecoderRNN, self).__init__()
        self.lang = lang
        
        self.embedding = nn.Embedding(output_size, embed_size)
        self.embedding.weight.data.copy_(torch.from_numpy(
            self.create_embedding_matrix(output_size, embeddings_index, embed_size)))

        self.gru = nn.GRU(embed_size, hidden_size, batch_first=True)
        self.out = nn.Linear(hidden_size, output_size)

    def create_embedding_matrix(self, output_size, embeddings_index, embed_size):
        matrix = np.zeros((output_size, embed_size))
        for word, idx in self.lang.word2index.items():
            embedding_vector = embeddings_index.get(word)
            if embedding_vector is not None:
                matrix[idx] = embedding_vector
...
encoder = EncoderRNN(input_lang.n_words, hidden_size, input_lang, embeddings_index, embed_size).to(device)
decoder = DecoderRNN(hidden_size, output_lang.n_words, output_lang, embeddings_index, embed_size).to(device)
