...
def trainIters(encoder, decoder, n_epochs, train_pairs, val_pairs, batch_size=64, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    train_dataloader = DataLoader(train_pairs, batch_size=batch_size, shuffle=True, collate_fn=tensorsFromPair)

    for epoch in range(1, n_epochs + 1):
        print(f"Epoch {epoch}/{n_epochs}")
        for i, (input_tensor, target_tensor) in enumerate(train_dataloader):
            loss = train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion)
            print_loss_total += loss * input_tensor.size(1)  # Multiply by batch size to get total loss
        ...
        print_loss_avg = print_loss_total / len(train_pairs)  # Divide by total number of pairs to get average loss
        print_loss_total = 0
        ...
        with open('translations_per_epochPau.json', 'w') as json_file:
